...
public class Receiver implements Runnable {
	...
	public Receiver(Sender sender, String srcIPAddress, Node parent, RouteTableManager routeTableManager) throws SocketException, UnknownHostException, BindException {
		this.parent = parent;
		...
		udpReceiver = new UdpReceiver(this, srcIPAddress);
	...
	public void run() {
			while (keepRunning) {
				try {
					...
					if(!msg.senderNodeAddress.equals(srcIPAddress)){
						try {
							switch (msg.getType()) {
	 ...
	 */
	public void addMessage(String senderNodeAddress, byte[] msg) {
	 ...
	 */
	private void routeRequestReceived(RREQ rreq, String senderNodeAddress) {
		...
		RREP rrep = null;
		try {
			if (rreq.getDestinationAddress().equals(srcIPAddress)) {
	...
	private void userDataPacketReceived(UserDataPacket userData) {
		int destlastIPAddress = Integer.parseInt(userData.getDestinationAddress().split("\\.")[3]);
		if (userData.getDestinationAddress().equals(srcIPAddress) 
				|| destlastIPAddress == Constants.BROADCAST_ADDRESS	) {
	...
	private class Message {
		...
		public Message(String senderNodeAddress, byte[] data) {
