...
public class Node extends Observable implements Runnable {
	 ...
	 */
    public Node(String srcIPAddress) throws InvalidNodeAddressException, SocketException, UnknownHostException, BindException {
    	this.srcIPAddress = srcIPAddress;
    	this.nodeAddress = Integer.parseInt(srcIPAddress.split("\\.")[3]);
    	...
    	routeTableManager = new RouteTableManager(srcIPAddress, this);
        sender = new Sender(this, srcIPAddress, routeTableManager);
        receiver = new Receiver(sender, srcIPAddress, this, routeTableManager);
...
//     	   sender.createNewRREQ(destinationAddress, Constants.UNKNOWN_SEQUENCE_NUMBER, false);
//        }
//    	sender.queueUserMessageFromNode(new UserDataPacket(packetIdentifier,destinationAddress, data, nodeAddress));
//    }
    
    public void sendData(String packetIdentifier, String destinationAddress, byte[] data){
    	if(!(destinationAddress.equals(srcIPAddress))) {
     	  sender.createNewRREQ(destinationAddress, Constants.UNKNOWN_SEQUENCE_NUMBER, false);
     	  
        }
    	sender.queueUserMessageFromNode(new UserDataPacket(packetIdentifier,destinationAddress, data, srcIPAddress));
    }
	 ...
	 */
	protected void notifyAboutDataReceived(String senderNodeAddess, byte[] data) {	
	 ...
	 */
	protected void notifyAboutRouteEstablishmentFailure(String faliedToReachAddress) {
	 ...
	 */
	protected void notifyAboutDataSentSucces(String packetIdentifier){
	...
	public class ValueToObserver implements MessageToObserver{
		...
		public ValueToObserver(String value, int msgType) {
			this.value = value;
			type = msgType;
		}
		@Override
	...
	public class PacketToObserver implements MessageToObserver{
		...
		public PacketToObserver(String senderNodeAddress, byte[] data, int msgType) {
			type = msgType;
			this.data = data;
		 ...
		 */
		public String getSenderNodeAddress(){
