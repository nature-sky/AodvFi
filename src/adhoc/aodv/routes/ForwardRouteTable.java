...
public class ForwardRouteTable {
	...
	private HashMap<String, ForwardRouteEntry> entries;
	...
	public ForwardRouteTable() {
		...
		entries = new HashMap<String, ForwardRouteEntry>();
	 ...
	 */
	public boolean removeEntry(String destAddress){
		synchronized (tableLock) {
	 ...
	 */
	public int getLastKnownDestSeqNumber(String destinationAddress) throws NoSuchRouteException{
	...
	public ArrayList<String> getPrecursors(String destinationAddress){
		synchronized (tableLock) {
	 ...
	 */
	public void setValid(String destinationAddress, int destinationSeqNumber, boolean validValue) throws NoSuchRouteException {
		ForwardRouteEntry entry = entries.get(destinationAddress);
		if(entry != null){
	 ...
	 */
	public ForwardRouteEntry getForwardRouteEntry(String destinationAddress) throws NoSuchRouteException, RouteNotValidException {
		ForwardRouteEntry entry = entries.get(destinationAddress);
		if (entry != null) {
			entry.resetAliveTimeLeft();
			synchronized (tableLock) {	
	 ...
	 */
	public ArrayList<RERR> findBrokenRoutes(String brokenNodeAddress){
