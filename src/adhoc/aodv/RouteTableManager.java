...
public class RouteTableManager {
	...
	public RouteTableManager(String nodeAddress, Node parent) {
	 ...
	 * @return returns true if the route were created and added successfully.
	 */
	protected boolean createForwardRouteEntry(String destinationNodeAddress, String nextHopAddress, int destinationSequenceNumber,
			int hopCount, boolean notifyObserver) {
		return createForwardRouteEntry(destinationNodeAddress, nextHopAddress, destinationSequenceNumber, hopCount, new ArrayList<String>(), notifyObserver);
	}
	 ...
	 * @return returns true if the route were created and added successfully.
	 */
	protected boolean createForwardRouteEntry(String destinationNodeAddress, String nextHopAddress,
						int destinationSequenceNumber, int hopCount, ArrayList<String> precursorNodes, boolean notifyObserver) {
	...
	protected boolean routeRequestExists(String sourceAddress, int broadcastID) {
	 ...
	 */
	protected boolean validForwardRouteExists(String destinationAddress, int destinationSequenceNumber) {
	 ...
	 */
	protected RouteRequestEntry getRouteRequestEntry(String sourceAddress, int broadcastID, boolean removeEntry)
			throws NoSuchRouteException {

		return (RouteRequestEntry) routeRequestTable.getRouteRequestEntry(sourceAddress, broadcastID, removeEntry);
	}

	protected ForwardRouteEntry getForwardRouteEntry(String destinationAddress) throws NoSuchRouteException, RouteNotValidException {
	...
	protected boolean RemoveForwardRouteEntry(String destinationAddress) {
		return forwardRouteTable.removeEntry(destinationAddress);
	}

	protected int getLastKnownDestSeqNum(String destinationAddress) throws NoSuchRouteException {
		return forwardRouteTable.getLastKnownDestSeqNumber(destinationAddress);
	}
	
	protected ArrayList<String> getPrecursors(String destinaitonAdrress){
	 ...
	 */
	protected void setValid(String destinationAddress, int newDestinationSeqNumber) throws NoSuchRouteException {
		forwardRouteTable.setValid(destinationAddress, newDestinationSeqNumber,true);
	}
	
	protected void setInvalid(String destinationAddress, int newDestinationSeqNumber) throws NoSuchRouteException {
	 ...
	 */
	protected int getHopCount(String destinationAddress) throws NoSuchRouteException, RouteNotValidException {
	 ...
	 * @throws NoSuchRouteException thrown if no table information is known about the destination
	 */
	protected void setRouteRequestTimer(String sourceAddress, int broadcastID) throws NoSuchRouteException {
				...
				try {
					...
					try {
						...
						while (route.getAliveTimeLeft() <= System.currentTimeMillis()) {
							...
							if (route.getSourceAddress().equals(nodeAddress)) {
